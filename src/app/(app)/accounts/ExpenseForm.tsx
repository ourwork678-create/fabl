...
export function ExpenseForm() {
  ...
  if (!open) {
    return <button onClick={() => setOpen(true)} className="btn-secondary w-full justify-center">{t("acc.addExpense")}</button>;
  }
  ...
  return (
    <form onSubmit={submit} className="card space-y-3 p-4">
      <div className="grid grid-cols-2 gap-3">
        ...
        <input name="amount" type="number" step="0.01" required placeholder={t("acc.amount")} className="input" />
      </div>
